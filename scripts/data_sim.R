...
sim_data <- tibble(id = 1:total_rows)

# Generate the cluster_id variable - 16% of the sample are siblings
sim_data <- sim_data %>%
  mutate(fam_id = c(rep(1:round(total_rows*0.16, 0),each=2), ((round(total_rows*0.16, 0)*2)+1):total_rows ))

# Create a "sex" variable with the desired Male:Female ratio
male_ratio <- 0.523  # Desired Male:Female ratio
sim_data <- sim_data %>%
  mutate(sex = sample(c("Male", "Female"), total_rows, replace = TRUE, prob = c(male_ratio, 1 - male_ratio)))
...
sim_data <- sim_data %>%
  mutate(height_6mo = rnorm_pre(birth_length, mu = 67.74, sd = 2.71, r=0.3),
         ...
         height_6mo = ifelse(id%in%nmissing_6mo$id, height_6mo, NA),
...
# Generate the breastfeeding variable - NB, this is deliberately simulated to be non-normally distributed, in order
...
save(sim_data, file= "simdata.RData")
